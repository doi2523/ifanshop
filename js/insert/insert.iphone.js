  ...
  import { getDatabase, set, ref, onValue, get, child, remove, update, onChildAdded } from "https://www.gstatic.com/firebasejs/10.10.0/firebase-database.js";
...
function displayGrid(gridData) {
    ...
    if (!tr || tr.children.length === 4) { // Nếu không có thẻ tr hoặc thẻ tr hiện tại đã có 4 cột
...
function updateThongTin(){
    ...
    if (snapshot.exists()) {
        ...
        document.getElementById('update-to-gird').addEventListener('click', function (event) {
            event.preventDefault();
