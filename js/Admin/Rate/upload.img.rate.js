...
function UploadImageAndGetURL() {
  const fileInput = document.getElementById("file-form"); // Lấy tham chiếu đến ô input chứa file
  ...
  const storageRef = ref(storage, "Rate/" + file.name);
  uploadBytes(storageRef, file).then((snapshot) => {
    console.log("Tải ảnh lên thành công!");
    const imageRef = ref(storage, "Rate/" + file.name);
